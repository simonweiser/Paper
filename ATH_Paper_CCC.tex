...
The number of mobile phone users is increasing enormously and concurrently increases the desire of being available. Therefore it is nowadays a common question, how distractions by phone calls while driving can be reduced. To achieve this, it should be the caller, who should decide whether to call a person or not, depending on the context of the callee. By displaying information about the context of the callee, the responsibility, whether a call should be executed or not, is transferred to the initiator of the call. In this paper we describe a concept and an app implementation to reduce the communication in the car. The conducted user study confirmed our thesis of the context information being perceived correctly.
...
There are about 4.5 to 5 billion mobile phone users worldwide \cite{stat14} and about 80 \% of all drivers are owning a smartphone \cite{farm14}.

\begin{figure}[H]
	\centering
	\includegraphics[width=.8\linewidth]{figures/worldwide_phoneuser}
	\caption{Number of mobile phone users worldwide in billions from 2013-2018.} \cite{stat14}
	\label{fig:app_worldwide}
\end{figure}

The extensive use of smartphones in the daily life produces a huge desire to be "always on". Due to this desire, phone users sometimes experience situations where they are highly occupied by their smartphone although these situations require their full attention and reaction to the environment. For instance, when driving a vehicle in bad weather conditions. Even though it is known that the risk of of collision is four times higher during cell phone use \cite{iihs05}, drivers willingly accept this risk. Even legal prohibitions could not increase the safety. Therefore the safety has to be ensured by giving the responsibility to execute a call to the initiator, who is not driving. Since the initiator is located in the outside environment of the driver, he has no activity or context data about the person he wants to call. Hence there is need to transfer these information about the callee to the caller.

Context information could be:
\begin{itemize}
	\item \textit{In vehicle}\\
	Abstract information if the callee is driving or not.
	\item \textit{Vehicle type}\\
	The type of the vehicle, e.g. car or bicycle.
%	\item \textit{Road type}\\
%	If the contact is"in vehicle", then the road type could be relevant, e.g. driving through the city or on a motorway.
	\item \textit{Speed}\\
	The speed with which the contact is on the road.
%	\item \textit{Weather conditions}\\
%	The weather conditions at the location of the callee, e.g. sunny or rainy.
	\item \textit{Geographic location}\\
	The exact position or the approximate region.
	\item \textit{Speakerphone}\\
	The information if hands-free speaking is possible through speakerphone in the vehicle.
	\item \textit{And many more\dots (see table \ref{tab:cont})}
\end{itemize}

In the last decade there was a lot of research about this topic. In the following we will point some related work to show the wide field of the current research and as well as our concept and the implementation of the Android application we designed. After building the application we conducted a user study to prove our expectation.
...
person~A sends the context information to a server every time the context changes. Person~B requests the data from the server when it is required, e.g. when the contact is opened in the contacts list app (see figure \ref{fig:app_concept}).

\begin{figure}[H]
	\centering
		\includegraphics[width=.95\linewidth]{figures/concept}
		\caption{Concept of exchanging context information.}
	\label{fig:app_concept}
\end{figure}

\subsection{Implementation}

We extended the design of the build-in Android contacts app by displaying the context of the person someone wants to call. In this case and for our goal, context refers particularly to the following information:
\newline
